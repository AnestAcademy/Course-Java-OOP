...
Trong ví dụ trên, chúng ta đã sử dụng ba phương thức được xác định trước `main()`, `print()` và `max()`. Chúng ta đã sử dụng các phương thức này trực tiếp mà không cần khai báo vì chúng đã được xác định trước.  
