...
Ví dụ chúng ta cần tìm số `int` lớn nhất bằng bao nhiêu? Trong `class Integer` đã có sẵn thuốc tính `MAX_VALUE` lưu trữ giá trị lớn nhất của `int`, chúng ta chỉ cần gọi nó ra là xong, không cần làm gì hay tính toán gì thêm.
